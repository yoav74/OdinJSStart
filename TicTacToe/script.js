class Gridpiece {
  ...
  UpdateText() {
    ...
    if (this.state == "X") this.text.style.color = "Red";
    else if (this.state == "O") this.text.style.color = "Blue";
  }
}
...
Arr.forEach((element) => {
  ...
  const newPiece = new Gridpiece(Arr.indexOf(element));
...
function insert(index) {
  if (GridArr[index - 1].state == "empty") {
    ...
    CheckStatus();
    FilledPlaces++;
    if (FilledPlaces >= 9) UpdateWinner("Draw");
  } else {
...
function CheckStatus() {
  for (const combination of winningCombinations) {
    ...
    if (
      GridArr[a].state != "empty" &&
      GridArr[a].state === GridArr[b].state &&
      GridArr[a].state === GridArr[c].state
    ) {
...
function reset() {
  GridArr.forEach((element) => {
    element.state = "empty";
    element.UpdateText();
    Arr[GridArr.indexOf(element)].onclick = function () {
      insert(GridArr.indexOf(element) + 1);
    };
  });
  currTurn = "O";
  UpdateUpperText();
